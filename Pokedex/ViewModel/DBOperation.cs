...
namespace Pokedex
{
    ...
    {
        public IEnumerable<Pokemon> GetDB()
        {
            ...
            var Pokemons = (from PokemonUnit in db.Pokemon
        ...
        {
            DBOperation reader = new DBOperation();
            var Pokemons = reader.GetDB();
            empty.Clear();
            var ActualPage = Index;
            var onePageOfPokemons = Pokemons.ToPagedList(ActualPage, 10);
        ...
        public static ObservableCollection<Pokemon> SearchDBByID(ObservableCollection<Pokemon> empty, int Index)
        {
            ...
            var Pokemons = reader.GetDB();
            empty.Clear();
            foreach (var item in Pokemons.Where(m => m.id == Index))
            {
                empty.Add(item);
            }
        ...
        public static ObservableCollection<Pokemon> SearchDBByName(ObservableCollection<Pokemon> empty, string nameRequested)
        {
            ...
            var Pokemons = reader.GetDB();
            empty.Clear();
            foreach (var item in Pokemons.Where(n => n.name.Contains(nameRequested)))
            {
                empty.Add(item);
            }
        ...
        public static ObservableCollection<Pokemon> SearchDBByType(ObservableCollection<Pokemon> empty, string typeSelected, int TypeIndex)
        {
            DBOperation reader = new DBOperation();
            var Pokemons = reader.GetDB();
            var SelectByTypeName = (Pokemons.Where(m => m.types.Any(u => u.type.name == typeSelected))).ToList<Pokemon>();
            empty.Clear();
            var onePageOfPokemons = SelectByTypeName.ToPagedList(TypeIndex, 10);
            var Convert = onePageOfPokemons.ToList<Pokemon>();
